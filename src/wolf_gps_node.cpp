        ...
        new_frame_elapsed_time_(_new_frame_elapsed_time)
{
    ...
    gps_sensor_ptr_ = new SensorGPS(new StateBlock(_gps_sensor_p, true), //gps sensor position. for now is fixed,
                                    ...
                                    new StateBlock(_map_pose.tail(1), true));  // map orientation
...
void WolfGPSNode::process()
{
    ...
    rot.setFromTwoVectors(Eigen::Vector3d::UnitZ(), gps_sensor_ptr_->getMapPPtr()->getVector());

    /*
     * TODO
     * find geographic north and align x-axis to north+MapO
     *
     * TODO Now is east!!
     */

    tf::Transform T_map2ecef;
    T_map2ecef.setOrigin(tf::Vector3(gps_sensor_ptr_->getMapPPtr()->getVector()[0], gps_sensor_ptr_->getMapPPtr()->getVector()[1], gps_sensor_ptr_->getMapPPtr()->getVector()[2]));
    ...
    T_map2ecef.setRotation(tf::Quaternion(rot.x(), rot.y(), rot.z(), rot.w()) * tf::createQuaternionFromYaw(gps_sensor_ptr_->getMapOPtr()->getVector()[0]));
    ...
    debugOperator2D(getVehiclePose().head(2), getVehiclePose().tail(1),
                      gps_sensor_ptr_->getPPtr()->getVector(), gps_sensor_ptr_->getIntrinsicPtr()->getVector(),
                      gps_sensor_ptr_->getMapPPtr()->getVector(),
                      gps_sensor_ptr_->getMapOPtr()->getVector());
...
//    std::cout << std::setprecision(12);
//    std::cout << "\n~~~~ RESULTS INSIDE ~~~~\n";
//    std::cout << "|\map P: " << _map_p.transpose() << std::endl;// map position wrt ecef
//    std::cout << "|\map O: " << _map_o.transpose() << std::endl;// map orientation wrt ecef
...
//    std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n\n";
    ...
    Eigen::Matrix<WolfScalar, 3, 1> map_p(_map_p[0], _map_p[1], _map_p[2]);
    ...
    Eigen::Matrix<WolfScalar, 3, 1> sensor_p_map; // sensor position with respect to map frame
    ...
    WolfScalar p = WolfScalar(sqrt(_map_p[0] * _map_p[0] + _map_p[1] * _map_p[1]));
    WolfScalar th = WolfScalar(atan2(a * _map_p[2], b * p));
    WolfScalar lon = WolfScalar(atan2(_map_p[1], _map_p[0]));
    WolfScalar lat = WolfScalar(atan2((_map_p[2] + ep * ep * b * pow(sin(th), 3)), (p - esq * a * pow(cos(th), 3))));
    ...
    std::cout << "_map_p: " << _map_p[0] << ", " << _map_p[1] << ", "
        ...
        //        std::cout << "_map_p LLA degrees: " << lat * WolfScalar(180 / M_PI) << ", " << lon * WolfScalar(180 / M_PI) /*<< ", " << alt*/ << std::endl;
    ...
    R4(0, 0) = WolfScalar(cos(_map_o[0]));
    R4(0, 1) = WolfScalar(sin(_map_o[0]));
    R4(1, 0) = WolfScalar(-sin(_map_o[0]));
    R4(1, 1) = WolfScalar(cos(_map_o[0]));
...
{
//    std::cout << "%%%%  FIX " << fix_arrived_ << " ecef = (" << msg->x << ", " << msg->y << ", " << msg->z << ")\n";
    fix_arrived_++;
